...
export class MusicService {
  constructor(
    ...
    @InjectRepository(LastSong)
    private readonly lastsongRepository: Repository<LastSong>,
  ...
  async deleteMusic(id: any): Promise<void> {
    // Step 1: Delete all related records in pd_songs_music;
    // await this.pdRepository.delete({ songs: id });

    // Step 2: Delete the record in the music table;
  ...
  async deleteMultipleSongsFromCuration(curationId: number, musicIds: number[]): Promise<void> {
    // 트랜잭션 시작
    ...
    try {
      ...
      // 삭제할 곡 필터링
      curation.songs = curation.songs.filter((song) => !musicIds.includes(song.id));

      // 필터링된 곡으로 Curation 엔티티 업데이트
      await queryRunner.manager.save(curation);

      // Music 테이블에서 관련된 곡 삭제
      await queryRunner.manager.delete(Music, musicIds);
  ...
  async findAll(userId?: number): Promise<any[]> {
    ...
    return musicList.map((music) => ({
      ...music,
      isLiked: userId
        ? music.likes.some((like) => like.user.id === userId)
        : false,
    }));
  }
  ...
  async findSoundFactoryAlbum(): Promise<any> {
    try {
      ...
      return Object.entries(groupedByArtist).map(([artist, albums]) => ({
        artist,
        albums: Object.entries(albums).map(([album, tracks]) => ({
          album,
          tracks,
        })),
      }));
      ...
      throw new InternalServerErrorException(
        error,
        'Error occurred while fetching all albums by artists.',
      );
    }
  }
  ...
  async createMusic(createMusicData: MusicDataDto, user: User): Promise<Music> {
    ...
    const existMusics = await this.musicRepository.findOne({
      where: { permalink, user: { id: user.id } },
    });
  ...
  async unlikeSong(userId: number, musicId: number): Promise<void> {
    ...
    if (!like) {
      throw new NotFoundException(
        `Like not found for user ${userId} on music ${musicId}`,
      );
    }
  ...
  async findPlaylistLastSong(): Promise<LastSong[]> {
    return this.lastsongRepository.find({ relations: ['songs'] });
  }


  async findOne(id: number): Promise<Curation> {
    return this.curationRepository.findOne({
      where: { id },
      relations: ['songs'],
    });
  }
  ...
  async createLastsong(
    ...
    const newLastsong = this.lastsongRepository.create({
      name,
      cover,
      songs,
    });

    return await this.lastsongRepository.save(newLastsong);
  }

  async deleteLastsong(name: string): Promise<void> {
    const result = await this.lastsongRepository.delete({ name });
    if (result.affected === 0) {
  ...
  async getDownloadedMusicByUser(
    user: User,
  ): Promise<{ music: Music; downloadedAt: Date }[]> {
  ...
  async checkSubscription(userId: number): Promise<boolean> {
    const user = await this.userRepository.findOne({where: { id: userId}});
    return user ? user.hasSubscription : false;
  }


  async removeSongFromPlaylist(
    songId: number,  // songId는 삭제하려는 음악의 ID입니다.
    userId: number   // userId는 사용자의 ID입니다.
): Promise<void> {
    // 특정 사용자의 플레이리스트 항목을 가져옵니다.
    
    const userPlaylistEntry = await this.playlistRepository
        .createQueryBuilder('Playlist')
        .leftJoinAndSelect('Playlist.music', 'music') // 변경된 부분: 'song' 대신 'music'
        .leftJoinAndSelect('Playlist.user', 'user')   // 변경된 부분: 공백 수정 및 정확한 alias 사용
        .where('Playlist.id = :playlistId', { playlistId: songId }) // 특정 플레이리스트 항목 ID를 사용
        .andWhere('music.id = :songId', { songId })    // 특정 노래 ID를 조건으로 추가
        .andWhere('user.id = :userId', { userId })     // 특정 사용자 ID를 조건으로 추가
        .getOne();

    console.log(userPlaylistEntry);
    if (!userPlaylistEntry) {
        throw new NotFoundException('Song not found in user playlist');
    }

    // music 엔티티에서 특정 항목을 삭제하려는 경우
    // userPlaylistEntry.music가 배열이 아닌 경우 바로 삭제할 수 있습니다.
    // 만약, 여러 곡이 있는 경우, 아래의 `remove` 호출이 변경되어야 할 수 있습니다.
    await this.playlistRepository.remove(userPlaylistEntry);
}
}
